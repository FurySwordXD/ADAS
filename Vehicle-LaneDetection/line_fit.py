...
class LineFit:
	...
	def tune_fit(self, binary_warped, left_fit, right_fit):
		"""
	...
	def viz1(self, binary_warped, ret, save_file=None):
		"""
	...
	def viz2(self, binary_warped, ret, save_file=None):
		"""
		Visualize the predicted lane lines with self.margin, on binary warped image
		save_file is a string representing where to save the image (if None, then just display)
		"""
		# Grab variables from ret dictionary
		left_fit = ret['left_fit']
		...
		# self.margin = 50  # NOTE: Keep this in sync with *_fit()
	...
	def calc_curve(self, left_lane_inds, right_lane_inds, nonzerox, nonzeroy):
		"""
		Calculate radius of curvature in meters
		"""
		y_eval = self.height - 1  # 720p video/image, so last (lowest on screen) y index is 719
	...
	def calc_vehicle_offset(self, undist, left_fit, right_fit):
		"""
	...
	def final_viz(self, undist, left_fit, right_fit, m_inv, left_curve, right_curve, vehicle_offset):
		"""
		...
		color_warp = np.zeros((self.height, self.width, 3), dtype='uint8')  # NOTE: Hard-coded image dimensions
	...
	def lane_detect(self, img):
		try:            
			...
			left_curve, right_curve = self.calc_curve(left_lane_inds, right_lane_inds, nonzerox, nonzeroy)
...
# 	plt.imshow(unwarped, vmin=0, vmax=1)
# 	plt.show()
